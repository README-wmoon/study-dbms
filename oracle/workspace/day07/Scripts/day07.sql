...
/*STADIUM 테이블에서 홈 팀인 팀 정보 조회, 홈팀이 없어도 경기장은 모두 조회*/
SELECT NVL(TEAM_ID, '공용') 팀아이디, STADIUM_NAME FROM 
...
SELECT PW.TEAM_ID, MAX(WEIGHT) FROM 
(
	SELECT MAX(WEIGHT) FROM PLAYER
)PW JOIN (
	SELECT TEAM_ID FROM TEAM
	GROUP BY TEAM_ID 
)TW ON PW.TEAM_ID = TW.TEAM_ID 
GROUP BY PW.TEAM_ID;
...
SELECT NVL(DEPARTMENT_NAME, '매니저'), D.MANAGER_ID, FIRST_NAME, LAST_NAME, DEPARTMENT_NAME 
FROM DEPARTMENTS D
